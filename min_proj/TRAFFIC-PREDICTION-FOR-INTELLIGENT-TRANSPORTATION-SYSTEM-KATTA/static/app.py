...
traffic_dataset = [[10, 0, 1, 8], [10, 0, 2, 9], [
    10, 1, 1, 13], [5, 0, 1, 15], [5, 1, 5, 16], [5, 1, 6, 18]]
traffic_status = [8, 7, 5, 6, 9, 9]

clf1 = svm.SVC(gamma=0.001, C=100, )

clf1.fit(traffic_dataset, traffic_status)
...
def predict_status(road, direction, day, time):
    result = clf1.predict([[int(road), int(direction), int(day), int(time)]])
    if result <= 2:
        return "Predicted Traffic Status: Good"
    if result > 2 and result > 5:
        return "Predicted Traffic Status: Normal"
    if result >= 5:
        return "Predicted Traffic Status: Not Recommended"
